...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { orderId, content, prompt, variables, isPWA } = body;
    ...
    // If variables are provided directly (PWA case), use them
    let emailVariables = variables;

    // If variables are not provided, fetch from database
    if (!emailVariables) {
    ...
    const formattedHtml = formatEmailContent(emailContent, emailVariables);

...
export async function POST(request: NextRequest, context: any): Promise<NextResponse> {
  ...
  const orderIdStr = context && context.params && context.params.orderId ? context.params.orderId : request.nextUrl.pathname.split('/')[4];
  ...
  // Log processed order items for debugging
